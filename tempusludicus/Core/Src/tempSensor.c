...
void init_adc_lm35(void)
{
    ...
    // Enable clock to Port D (PTD1_SE5b)
...
{
    // Convert ADC result to voltage (assuming ADC reference voltage is 3.01V = 3010 mv)
...
float calculateAverageTemperature(void)
{
    ...
    if (validCount > 0) {
        ...
        return roundf(averageTemperature * 2.0f) / 2.0f;
